...
public class ConnectedSocket extends Thread {
	...
	private void requestMapping(String requestJson) {
		...
		case "sendMessage":
			...
			sendToAll(new ResponseDto<String>("receiveMessage", username + " :" + requestDto.getBody()),
					room.getUsers());
			break;
		case "exitRoom":
			room = findConnectedRoom(username);
	...
	private void sendToAll(ResponseDto<?> responseDto, List<ConnectedSocket> connectedSockets) {
		for (ConnectedSocket connectedSocket : connectedSockets) {
			try {
				OutputStream outputStream = connectedSocket.getSocket().getOutputStream(); // connectedSocket 안에 있는 접속한
																							// socket에게 뿌려줘야함
				PrintWriter printWriter = new PrintWriter(outputStream, true);
